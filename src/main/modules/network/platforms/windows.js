...
/**
 * Windows platform-specific network operations
 */
class WindowsNetworkChecker {
    /**
     * Check if a domain is accessible on Windows using ping
    ...
    static async checkDomainStatus(domain) {
        console.log("=== Windows Network Check ===");
            ...
            return new Promise((resolve) => {
                ...
                ping.on("close", (code) => {
                    ...
                    if (code === 0) {
                        ...
                        // Look for "time=XXms" or "time<XXms" pattern
                        const timeMatch = output.match(/time[=<](\d+)ms/i);
                        const pingTime = timeMatch ? parseInt(timeMatch[1]) : responseTime;
                        ...
                        if (output.includes("无法访问目标主机") || output.includes("Destination host unreachable")) {
                            errorMessage = "目标主机不可达";
                        } else if (output.includes("请求超时") || output.includes("Request timed out")) {
                            errorMessage = "请求超时";
                        } else if (output.includes("找不到主机") || output.includes("could not find host")) {
                            errorMessage = "找不到主机";
                        }
...
module.exports = WindowsNetworkChecker;
